...
def main():
    ...
    """,unsafe_allow_html=True)
        ...
        if match_id :
            ...
            elif choice == 'Team performance':
                
                choice_player,_,choice_event,_,sides = st.beta_columns([2.3,0.5,2,0.5,4])
                player = choice_player.selectbox("Choose a player",
                                                match_events.sort_values(by=['team','shortName'])['shortName'].unique().tolist(),
                                                format_func=lambda x : f"{x} ({match_events.set_index('shortName').loc[x,'team'][0]} {match_events.set_index('shortName').loc[x,'role'][0]})"
                                                )
                sides.text_input("Sides at the start",value=f"{left_team} - {right_team}")
                stats,pitches = st.beta_columns([450,700])
                event_name = choice_event.selectbox("Choose an event",match_events['eventName'].unique())
                fig,event = plot_events_scatter(match_info,match_events,teams_id_name,match_id,event_name, ['ALL'] , ['ALL'],player=player,outcomes=['BOTH'])
                
                fig_passes = px.bar(event.groupby(['subEventName','tag']).size().reset_index(name='count').sort_values(by='count',ascending=False),
                        x='subEventName',
                        y='count',
                        log_y=True,
                        text='count',
                        color='tag',
                        barmode='relative',
                        color_discrete_map={'accurate':'darkgreen',
                                            'not accurate':'darkred',
                                            'interception':'royalblue',
                                            'lost':'darkred',
                                            'won':'darkgreen',
                                            'neutral':'lightgrey',
                                            },
                        category_orders={'tag':['accurate','not accurate','interception']},
                        labels={'subEventName':''},
                        height=600,
                        width=450,
                        template='seaborn',
                        )
                fig_passes.update_yaxes(visible=False)
                fig_passes.update_xaxes(showgrid=False)
                fig_passes.update_layout(title=dict(x=0.5),legend=dict(
                orientation='v',xanchor='right',yanchor='top'))
                #stats.markdown("<br> <br> <br> <br> <br> <br> <br> ",unsafe_allow_html=True)
                stats.plotly_chart(fig_passes)

                fig.update_layout(title=dict(text=f"{player} ({event[event['shortName']==player]['team'].iloc[0]}) {event_name}",x=0.5),
                                  legend=dict(orientation='h'),
                                  )
               
                pitches.plotly_chart(fig)
                
                with st.beta_expander("Radar Chart"):
                    group = match_events.query("(tag!='')&(eventName!=['Pass','Duel'])").groupby(['team','eventName']).size().reset_index(name='count')
                    #group['acc'] = match_events.query("tags==['accurate','interception_accurate']").groupby(['team','eventName']).size().values
                    #group['acc'] = match_events.query("")
                    st.write(match_events[(match_events['tag'].str.contains('interception'))&(match_events['team']=='Liverpool')])
                    mask = (match_events['tag'].str.contains('interception'))&(match_events['eventName']!='Pass')
                    test = pd.DataFrame([])
                    test['fouls'] = match_events.query("eventName=='Foul'").groupby(['team']).size()/match_events.query("eventName=='Foul'").shape[0]
                    test['intercept_acc'] = match_events.query("(eventName!='Pass')&(tag==['interception_accurate','interception'])").groupby(['team']).size()/match_events.query("@mask").shape[0]
                    test['pass_acc'] = match_events.query("(eventName=='Pass')&(tag==['interception_accurate','accurate'])").groupby(['team']).size()/match_events.query("(eventName=='Pass')").groupby(['team']).size()
                    test['duel_acc'] = match_events.query("(eventName=='Duel')&(tag==['interception_accurate','accurate'])").groupby(['team']).size()/match_events.query("(eventName=='Duel')").groupby(['team']).size()
                    test['shot_acc'] = match_events.query("(eventName=='Shot')&(tag==['Goal','accurate'])").groupby(['team']).size()/match_events.query("(eventName=='Shot')").groupby(['team']).size()
